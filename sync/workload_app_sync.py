...
def create(session_id, clientid, explen):
    ...
    while time.time() < t_end:
        ...
        if app_request_type < 10:
            ...
            utils_app_sync.update_company_market_capitalization(session_id, amount, symbol)
        elif app_request_type < 40:
            selector = 1
            sort_key = random.choice(["top10", "bottom10"])
            utils_app_sync.get_ranks_by_sort_key(session_id, sort_key)
        elif app_request_type < 70:
            selector = 2
            selected_symbols = random.sample(symbols, 2)
            utils_app_sync.get_ranks_by_symbols(session_id, selected_symbols)
        else:
            selector = 3
            start_index = random.randint(0, 10)
            end_index = start_index + random.randint(1, 5)
            utils_app_sync.get_zrange(session_id, start_index, end_index)
